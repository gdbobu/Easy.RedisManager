...
namespace Easy.RedisManager.RedisAccess.Communication
{
    ...
    ///     Redis通信客户端（Socket版）
    /// </summary>
    public class RedisSocketClient1 : IDisposable
    {
        #region Constant
        ...
        public RedisSocketClient1()
        {
        }

        public RedisSocketClient1(string host, int port)
        {
            Host = host;
            Port = port;
        }

        public RedisSocketClient1(string host, int port, string password)
            :this(host, port)
        {
            Password = password;
        }
