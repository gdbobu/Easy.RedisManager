...
namespace Easy.Common
{
        ...
        where TSerializer : ITypeSerializer
    {
        ...
        public static object ParsePrimitive(string value)
        {
            ...
            var acceptDecimal = JsConfig.ParsePrimitiveFloatingPointTypes.HasFlag(EnumParseAsType.Decimal);
            ...
            if (isDecimal && decimalValue == decimal.Truncate(decimalValue))
            {
                ...
                if (parseAs.HasFlag(EnumParseAsType.Byte) && decimalValue <= byte.MaxValue && decimalValue >= byte.MinValue) return (byte)decimalValue;
                if (parseAs.HasFlag(EnumParseAsType.SByte) && decimalValue <= sbyte.MaxValue && decimalValue >= sbyte.MinValue) return (sbyte)decimalValue;
                if (parseAs.HasFlag(EnumParseAsType.Int16) && decimalValue <= Int16.MaxValue && decimalValue >= Int16.MinValue) return (Int16)decimalValue;
                if (parseAs.HasFlag(EnumParseAsType.UInt16) && decimalValue <= UInt16.MaxValue && decimalValue >= UInt16.MinValue) return (UInt16)decimalValue;
                if (parseAs.HasFlag(EnumParseAsType.Int32) && decimalValue <= Int32.MaxValue && decimalValue >= Int32.MinValue) return (Int32)decimalValue;
                if (parseAs.HasFlag(EnumParseAsType.UInt32) && decimalValue <= UInt32.MaxValue && decimalValue >= UInt32.MinValue) return (UInt32)decimalValue;
                if (parseAs.HasFlag(EnumParseAsType.Int64) && decimalValue <= Int64.MaxValue && decimalValue >= Int64.MinValue) return (Int64)decimalValue;
                if (parseAs.HasFlag(EnumParseAsType.UInt64) && decimalValue <= UInt64.MaxValue && decimalValue >= UInt64.MinValue) return (UInt64)decimalValue;
                return decimalValue;
            }
            ...
            float floatValue;
            var acceptFloat = JsConfig.ParsePrimitiveFloatingPointTypes.HasFlag(EnumParseAsType.Single);
            var isFloat = float.TryParse(value, NumberStyles.Float, CultureInfo.InvariantCulture, out floatValue);
            if (acceptFloat && isFloat)
                return floatValue;

            double doubleValue;
            var acceptDouble = JsConfig.ParsePrimitiveFloatingPointTypes.HasFlag(EnumParseAsType.Double);
