    ...
    public class TcpChannel
    {
        ...
        public int Send(byte[] bts)
        {
            if (socket == null || !socket.Connected)
            {
                if (onSocketNullOrNotConnected != null)
                    onSocketNullOrNotConnected(this
                        , new TcpChannelNotifyEventArgs(EnumNotificationLevel.Error, "Send"));
            ...
            catch (Exception e)
            {
                if (onSyncSendException != null)
                    onSyncSendException(this
                        , new TcpChannelNotifyEventArgs(EnumNotificationLevel.Error, "socket.Send, Sync.", null, e));
        ...
        public bool AsyncSend(byte[] bts)
        {
            if (Interlocked.CompareExchange(ref failureToSend, 0, 0) != 0)
            {
                if (onAsyncSendFailureToSend != null)
                    onAsyncSendFailureToSend(this
                        , new TcpChannelNotifyEventArgs(EnumNotificationLevel.Error, "AsyncSend"));
            ...
            if (Interlocked.CompareExchange(ref failureToSend, 0, 0) == 0)
            {
                try
                {
                    ...
                    br = socket.BeginSend(bts, 0, bts.Length, SocketFlags.None, new AsyncCallback(ar =>
                    {
                        ...
                        if (onInfo != null)
                            onInfo(this, new TcpChannelNotifyEventArgs(EnumNotificationLevel.Debug, "AsyncSend, Socket.BeginSend Callback",
                        ...
                        if (ex != null || sent == 0 || sent < bts.Length || totalMs > socket.SendTimeout)
                        {
                            ...
                            TcpChannelNotifyEventArgs e = new TcpChannelNotifyEventArgs(EnumNotificationLevel.Error, "AsyncSend, Socket.BeginSend Callback", dctData, ex);
                ...
                catch (Exception e)
                {
                    if (onAsyncSendException != null)
                        onAsyncSendException(this
                            , new TcpChannelNotifyEventArgs(EnumNotificationLevel.Error, "AsyncSend, Socket.BeginSend Callback"
                                , string.Format("ERROR in TcpChannel \"{0}\": Failed to send {1} bytes. ", Id, bts.Length), e));
                ...
                if (socket == null)
                {
                    if (onAsyncSendFailureToSend != null)
                        onAsyncSendFailureToSend(this
                            , new TcpChannelNotifyEventArgs(EnumNotificationLevel.Error, "AsyncSend, Socket.BeginSend Callback"
                                , string.Format("ERROR: TcpChannel \"{0}\": Failed to send {1} bytes due to invalid socket.", Id, bts.Length)));
        ...
        static public IPAddress LocalIP
        {
            get
            {
                return localIP != null
                        ? localIP
                        : Dns.GetHostAddresses(Dns.GetHostName()).Where(
                        item => item.AddressFamily == AddressFamily.InterNetwork && !item.IsIPv6LinkLocal).ToArray()[0];
            }
        }
        ...
        static public IPAddress GetIPAddress(string hostAddressOrName)
        {
            ...
            if (!string.IsNullOrEmpty(hostAddressOrName) && hostAddressOrName.ToLower() != DEFAULT_LOCAL_HOST_NAME)
            {
                ...
                if (ipAddr == null)
                    ipAddr = Dns.GetHostEntry(hostAddressOrName).AddressList.Where(
                        addr => addr.AddressFamily == AddressFamily.InterNetwork).ToArray()[0];
            }
            else
                ipAddr = LocalIP;
